...
title: "T197986"
description: | 
  Notes on quantifying the state of Wikidata incompleteness and inconsistency
output:
  html_notebook:
    theme: readable
    code_folding: hide
...
This project (tracked in [T197986](https://phabricator.wikimedia.org/T197986)) is about quantifying the state of inconsistency and incompleteness of Q-items on Wikidata. The following are examples which were highlighted in the deck [Wikidata completeness and quality issues](https://docs.google.com/presentation/d/1dANu94y9AA16t6fN_i_hagqlsdI92qMmezhIb14F-bo/edit?usp=sharing):
...
```
```{r n_taxons, message=FALSE, cache=TRUE, echo=FALSE}
...
Let's take a look at a few taxons to get a sense of what data they may have:

```{r example_taxons, echo=FALSE}
extract_en_info_field <- function(info_list, field) {
  if (!"en" %in% names(info_list)) return(NA)
  else return(info_list$en[[field]])
}
taxons %>%
  filter(entity %in% c("Q28319", "Q311230", "Q1000270", "Q1034859", "Q1035244", "Q25327", "Q29995")) %>%
  arrange(desc(entity)) %>%
  mutate(
    label = map_chr(info, extract_en_info_field, field = "label"),
    label = ifelse(is.na(label), "—", label),
    description = map_chr(info, extract_en_info_field, field = "description"),
    description = ifelse(is.na(description), "—", description),
    item = sprintf("%s (%s)", label, wikidata_link(entity)),
    aliases = map_chr(en_aliases, maybe_make_list),
    taxon_common_names = map_chr(en_taxon_common_names, maybe_make_list)
  ) %>%
...
When the items are indexed, the index includes labels (if any), descriptions (if any), aliases (if any), and any values of statements which are plain text or Q-items. So if an item is, say, a person who is an instance of (P31) of human (Q5), then their index includes "P31=Q5" but *not* that they're a "human". However, if the value is plain text (e.g. taxon common name), then that gets included in the index and can be searched for. For example, Wikimedia Foundation, Inc. ([Q180](https://www.wikidata.org/wiki/Q180)) has a statement for property "
IPv4 routing prefix" ([P3761](https://www.wikidata.org/wiki/Property:P3761)) so if one does a [full-text search for "198.35.26.0/23"](https://www.wikidata.org/w/index.php?search=198.35.26.0%2F23&title=Special%3ASearch&fulltext=1) (the value currently in that statement), WMF is the first result listed.

So one way to assess the searchability of taxons on Wikidata is to assess how many have lebels, descriptions, aliases, and taxon common names ([P1843](https://www.wikidata.org/wiki/Property:P1843)). Among `r nrow(taxons)` taxons, we have following missingness statistics:

```{r, echo=FALSE}
taxon_completeness <- taxons %>%
  select(en_aliases, en_taxon_common_names, info) %>%
  transmute(
    `alias(es)` = !is.na(en_aliases),
    `taxon common name(s)` = !is.na(en_taxon_common_names),
    label = map_lgl(map(info, flatten), ~ !is.na(.x$label)),
    description = map_lgl(map(info, flatten), ~ !is.na(.x$description)),
  ) %>%
  gather(has, val) %>%
  group_by(has) %>%
  summarize(n = sum(val), total = n())
taxon_completeness_n <- set_names(taxon_completeness$n, taxon_completeness$has)
taxon_completeness %>%
  transmute(has = has, prop = sprintf("%s (%.1f%%)", compress(n), 100 * n / total)) %>%
  kable(col.names = c("Information a taxon item may have", "How many have a value (in English)"),
        caption = sprintf("English completeness of items among a subset of %s taxons", compress(nrow(taxons))),
        align = c("l", "r")) %>%
  kable_styling("striped")
```

Again, these numbers are *not* representative of all `r compress(n_taxons)` taxons. As a reminder, the dataset of `r compress(nrow(taxons))` taxons studied was limited to those which had (1) at least one English alias or (2) at least one English taxon common name. It's not clear how many taxons on Wikidata do have a description or a label, but we can at least say that of `r compress(n_taxons)`, only `r compress( taxon_completeness_n["alias(es)"])` (`r sprintf("%.2f%%", 100 * taxon_completeness_n["alias(es)"] / n_taxons)`) taxons have at least one alias in English and only `r compress(taxon_completeness_n["taxon common name(s)"])` (`r sprintf("%.2f%%", 100 * taxon_completeness_n["taxon common name(s)"] / n_taxons)`) taxons have at least one taxon common name in English.

**Note**: The next iteration of this work should include all languages. We restricted this initial exploration to English as that is the analyst's primary language.

## Groups of organisms known by one particular common name

```{r example_commons, echo=FALSE}
commons %>%
